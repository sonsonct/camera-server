...
export class AuthService {
  ...
  async registerAdmin(registerAdminDto: RegisterAdminDto) {
    ...
    const adminInsert = await this.userRepository.insert({
  ...
  async register(registerDto: RegisterDto) {
    ...
    return await this.processSendOtp({
  ...
  async loginSocial(userData: LoginSocialDto) {
    try {
      ...
      if (social) {
        user = await this.userRepository.findOneBy({ id: social.userId });
      } else {
        const username = await this.generateUniqueUsername();
        ...
        await this.socialLoginRepository.insert({
          socialId: id,
          userId,
  ...
  async blockUser(blockDto: any) {
    if (blockDto.userId) {
      const user = await this.userRepository.findOneBy({
        id: blockDto.userId,
        role: Not(RoleScope.ADMIN),
      });
  ...
  async loginFacebook(userData: LoginSocialDto) {
    try {
      const { data } = await firstValueFrom(
        this.httpService.get(
          `https://graph.facebook.com/v19.0/${userData.userData['dataUser'].user.id}?access_token=${userData.userData['dataUser'].accessToken}`,
        ),
      );
      return data.id;
    } catch (error) {
